...
const static int MAX_CLIENTS = 10000;
...
float point_cloud[MAX_CLIENTS * 2 + MAX_NPC * 2];
...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_MOVE_OBJECT: {
		...
		if (move_packet->id < MAX_CLIENTS) {
			...
			if (-1 != my_id) {
				g_clients[my_id].x = move_packet->point.x;
				g_clients[my_id].y = move_packet->point.y;
			}
			if (ci == my_id) {
		...
		else if (move_packet->id < MAX_CLIENTS + MAX_NPC) {
			g_npcs[move_packet->id].x = move_packet->point.x;
			g_npcs[move_packet->id].y = move_packet->point.y;
		}
	}
					   break;
	case SC_ADD_OBJECT:{
		...
		if (add_packet->id >= MAX_CLIENTS) {
			g_npcs[add_packet->id]._visible = true;
			active_npcs++;
		}
	}
		break;
	case SC_REMOVE_OBJECT: {
		...
		if (remove_packet->id >= MAX_CLIENTS) {
			g_npcs[remove_packet->id]._visible = false;
			active_npcs--;
		}
	...
	case SC_LOGIN_INFO:
	{
		...
		g_clients[my_id].x = login_packet->point.x;
		g_clients[my_id].y = login_packet->point.y;
	}
	break;
...
void GetPointCloud(int* size, float** points)
{
	...
	for (int i = 0; i < MAX_NPC; ++i)
		if (true == g_npcs[i]._visible) {
			point_cloud[index * 2] = static_cast<float>(g_npcs[i].x);
			point_cloud[index * 2 + 1] = static_cast<float>(g_npcs[i].y);
			index++;
		}
